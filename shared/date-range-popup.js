...
class DateRangePopup extends HTMLElement{
  constructor(){ super(); this.attachShadow({mode:'open'}); this.view=new Date(); this.start=null; this.end=null; }
  connectedCallback(){ this.render(); }
  setRange(s,e){ this.start=s?new Date(s):null; this.end=e?new Date(e):null; this.renderGrid(); }
  ...
  render(){
    ...
    `;
    s.querySelector('.grid.head').innerHTML = ['Pn','Wt','Śr','Cz','Pt','So','Nd'].map(x=>`<div class="cell" style="font-weight:700;cursor:default">${x}</div>`).join('');
  ...
  renderGrid(){
    ...
    for(let i=0;i<42;i++){
      ...
      const inSel = this.start && this.end && d>=new Date(toISODate(this.start)) && d<=new Date(toISODate(this.end));
      days.push(`<div class="cell ${dim} ${inSel?'in':''} ${isSel?'sel':''}" data-date="${d.toISOString()}">${d.getDate()}</div>`);
    }
